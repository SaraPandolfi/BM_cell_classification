...
from parameters import train_params, test_img_path, img_path, num_classes, classes
import pytest

@pytest.fixture(scope="module")
def data_generators():
    """
    This fixture function creates training, validation and test set generators using the
...
def test_dataset_generator_returns_tuple(data_generators):
    """
    This test checks if the dataset_generator function returns three objects of type tf.data.Dataset,
    which are used for training, validation and testing the model respectively.
    """
...
def test_dataset_generator_length(data_generators):
    """
    This test checks if the length of the training, validation and test sets generated are greater than zero.
    """
    train_set, val_set, test_set = data_generators
...
def test_dataset_generator_image_shape_dtype(data_generators):
    ...
    assert x_train.shape == tf.TensorShape([train_params['batch_size'], train_params['image_size'][0], train_params['image_size'][1], 3])
